...
def get_exports_data():
    ...
    end_time = time.time()
    logger.debug(
        f"Finalizado download dos dados de exportação - time: {calculateTime(end_time, start_time)}"
    )
    return df
...
def get_imports_data():
    ...
    end_time = time.time()
    logger.debug(
        f"Finalizado download dos dados de importação - time: {calculateTime(end_time, start_time)}"
    )
    return df
